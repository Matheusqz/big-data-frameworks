...
docker-compose exec -d resourcemanager apt-get -y install python
docker-compose exec nodemanager1 apt-get update
docker-compose exec -d nodemanager1 apt-get -y install python
docker-compose exec nodemanager2 apt-get update
docker-compose exec -d nodemanager2 apt-get -y install python
docker-compose exec nodemanager3 apt-get update
docker-compose exec -d nodemanager3 apt-get -y install python
